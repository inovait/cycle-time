...
const getUsersThatWorkedOnIssue = (changelog, assignee, issueStartDate, issueEndDate) => {
    ...
    if (allUsersAssignedBetweenImplementationOfIssue.length) {
        const assignee = allUsersAssignedBetweenImplementationOfIssue
            .reverse()
            .map(({ items }) => items.filter(({ field }) => field === 'assignee')
...
exports.default = getUsersThatWorkedOnIssue;
